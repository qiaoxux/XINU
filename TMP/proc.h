...
struct	pentry	{
	...
	struct plentry plocks[NLOCKS];		/* to record all locks' condition	 */

	int pinh;		/* The current inherited priority of the process. This value can be 0 
					when the process is running with its original priority.			*/
	int lockid;		/* lock descriptor in whose wait queue the process is blocked. It can 
					take the value ­1 when the process is not blocked inside any wait queue. 
					Note that a process can be only inside a single wait queue at a time.		*/
};
