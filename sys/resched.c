...
int write_data(int pid)
{
	...
	for(i = 0; i < NFRAMES; i++){
		if(frm_tab[i].fr_pid == pid && frm_tab[i].fr_vpno > 4096 && frm_tab[i].fr_type == FR_PAGE) {	
         ...
         if(bs_id >= 0 && bs_id <=7)
         	write_bs((char *)((i + FRAME0) * NBPG), bs_id, (frm_tab[i].fr_vpno - bsm_tab[proctab[pid]].vpno));
      }
   }   
...
int read_data(int pid)
{
	...
	for(i = 0; i < NFRAMES; i++){
		if(frm_tab[i].fr_pid == pid && frm_tab[i].fr_vpno > 4096 && frm_tab[i].fr_type == FR_PAGE) {	
         ...
         if(bs_id >= 0 && bs_id <=7)
         	read_bs((char *)((i + FRAME0) * NBPG), bs_id, (frm_tab[i].fr_vpno - bsm_tab[proctab[pid]].vpno));
      }
   }   
